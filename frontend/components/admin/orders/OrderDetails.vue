    ...
    <div class="order-info">
        ...
        <template v-else-if="currentOrder">
            <div class="tw-space-y-8">
                ...
                <!-- Hoàn đơn hàng -->
                <div v-if="currentOrder.return_status" class="tw-mt-4 tw-p-4 tw-rounded-lg tw-border-l-4 tw-flex tw-items-center" :class="getReturnStatusContainerClass(currentOrder.return_status)">
                    <div :class="['tw-w-8','tw-h-8','tw-rounded-full','tw-flex','tw-items-center','tw-justify-center','tw-mr-3', getReturnStatusIconClass(currentOrder.return_status)]">
                      <svg :class="['tw-w-5','tw-h-5', getReturnStatusTextClass(currentOrder.return_status)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M9 15l-6-6 6-6M3 9h9a6 6 0 016 6v3" />
                      </svg>
                    </div>
                    <div>
                        <p class="tw-font-medium tw-text-sm md:tw-text-base" :class="getReturnStatusTextClass(currentOrder.return_status)">
                            {{ getReturnStatusLabel(currentOrder.return_status) }}
                        </p>
                        <p v-if="currentOrder.return_status" class="tw-text-sm tw-mt-1" :class="getReturnStatusTextClass(currentOrder.return_status)">
                            {{ getReturnDateLabel(currentOrder.return_status) }}: {{ currentOrder.return_requested_at ? formatDate(currentOrder.return_requested_at) : formatDate(currentOrder.updated_at) }}
                        </p>
  ...
  <div class="tw-flex tw-items-center tw-gap-4 tw-mb-2 tw-justify-between">
    ...
    <div v-if="currentOrder.return_status === 'requested'" class="tw-flex tw-items-center tw-gap-2">
      ...
      <button @click="openRejectModal" class="tw-bg-red-600 tw-text-white tw-px-4 tw-py-2 tw-rounded hover:tw-bg-red-700">
        ...
        <div v-if="showRejectModal" class="tw-fixed tw-inset-0 tw-bg-black tw-bg-opacity-50 tw-flex tw-items-center tw-justify-center tw-z-50">
          <div class="tw-bg-white tw-rounded-lg tw-shadow-lg tw-p-6 tw-w-full tw-max-w-md">
            ...
            <div class="tw-flex tw-justify-end tw-gap-2">
              <button @click="showRejectModal = false" class="tw-bg-gray-200 tw-px-4 tw-py-2 tw-rounded">Hủy</button>
              <button @click="handleRejectReturn" class="tw-bg-red-600 tw-text-white tw-px-4 tw-py-2 tw-rounded hover:tw-bg-red-700">Xác nhận</button>
            </div>
          </div>
        </div>
    </div>
