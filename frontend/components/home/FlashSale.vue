  ...
  <div v-if="flashSaleProducts.length > 0" class="tw-bg-white tw-rounded tw-p-6 tw-mb-6">
    ...
    <div class="tw-relative">
      <button @click="scrollLeft"
        class="tw-absolute tw-left-0 tw-top-1/2 -tw-translate-y-1/2 tw-z-20 tw-bg-white tw-shadow tw-rounded-full tw-w-10 tw-h-10 tw-flex tw-items-center tw-justify-center hover:tw-bg-gray-100 tw-transition"
        style="outline:none;">
      ...
      <div ref="sliderRef" class="tw-flex tw-gap-4 tw-overflow-x-auto tw-scroll-smooth tw-px-12"
        style="scrollbar-width:none;">
        <NuxtLink v-for="product in flashSaleProducts" :key="product.id"
          ...
          class="tw-relative tw-w-64 tw-flex-shrink-0" style="text-decoration: none; color: inherit;">
          <div class="tw-relative tw-overflow-hidden tw-group tw-pb-2 sm:tw-pb-3 tw-bg-white"
            :style="`width: 250px; height: 370px; margin: 17px auto; background: url('${productSaleBg}') center/cover no-repeat;`">
            <div class="tw-relative tw-overflow-hidden tw-rounded-[5px] "
          ...
          <div class="tw-bg-white tw-px-2 tw-py-3 tw-shadow -tw-mt-4 tw-z-10 tw-relative">
            ...
            <div class="tw-flex tw-items-center tw-gap-1 tw-mb-1">
              ...
              <span v-if="(product.variants && getUniqueColors(product).length > 3)"
                class="tw-text-xs tw-text-gray-400">+{{ getUniqueColors(product).length - 3 }}</span>
            </div>
            <div class="tw-w-full tw-mt-2 tw-px-2">
              <div class="tw-relative tw-h-6 tw-bg-gray-200 tw-rounded-full">
                <div class="tw-absolute tw-left-0 tw-top-0 tw-h-6 tw-bg-blue-600 tw-rounded-full"
                  :style="`width: ${getSoldPercent(product)}%; transition: width 0.3s;`"></div>
                <div
                  class="tw-absolute tw-left-3 tw-top-0 tw-h-6 tw-flex tw-items-center tw-z-10 tw-text-white tw-font-semibold tw-text-sm">
      ...
      <button @click="scrollRight"
        class="tw-absolute tw-right-0 tw-top-1/2 -tw-translate-y-1/2 tw-z-20 tw-bg-white tw-shadow tw-rounded-full tw-w-10 tw-h-10 tw-flex tw-items-center tw-justify-center hover:tw-bg-gray-100 tw-transition"
        style="outline:none;">
        <svg xmlns="http://www.w3.org/2000/svg" class="tw-h-6 tw-w-6" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
    ...
    <div class="tw-flex tw-justify-center tw-mt-4">
      <NuxtLink to="/flash-sale" class="tw-border tw-px-6 tw-py-2 tw-rounded tw-bg-white tw-font-bold">Xem tất cả &gt;
      </NuxtLink>
    </div>
  </div>
</template>
