...
const hasEnabledPayment = computed(() =>
  paymentMethods.value.some((m) => m.enabled)
)
...
const updatePaymentMethods = () => {
  ...
  paymentMethods.value = [
    ...
    {
      title: 'VNPay',
      description: Number(s.enableVnpay) ? 'Thanh toán qua cổng VNPay' : 'Sắp ra mắt',
      code: 'vnpay',
...
const placeOrder = async () => {
  try {
    ...
    if (!method?.enabled) {
      error.value = 'Phương thức thanh toán này hiện chưa khả dụng.'
      return
    }
    ...
    if (result && result.order) {
      const paymentMethod = method.code
...
<template>
  <div class="tw-max-w-7xl tw-mx-auto tw-px-4 md:tw-px-6 tw-py-8">
    ...
    <div v-if="isLoading" class="tw-flex tw-justify-center tw-items-center tw-py-12">
      <div class="tw-animate-spin tw-rounded-full tw-h-12 tw-w-12 tw-border-t-2 tw-border-b-2 tw-border-[#81AACC]" />
    </div>
    ...
    <div v-else class="tw-grid tw-grid-cols-1 lg:tw-grid-cols-2 tw-gap-8">
      <div class="tw-space-y-8">
        <div class="tw-bg-white tw-p-6 tw-rounded-lg tw-shadow-sm">
          ...
          <div v-if="!hasEnabledPayment" class="tw-text-red-500 tw-text-sm tw-mt-2">
